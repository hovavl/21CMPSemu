...
#                 like = -(1 / 2) * (((val - lum_data[j]) / lum_err_inf[j]) ** 2 + np.log(2 * np.pi * lum_err_inf[j] ** 2))
#             else:
#                 like = -(1 / 2) * (((val - lum_data[j]) / lum_err_sup[j]) ** 2 + np.log(2 * np.pi * lum_err_sup[j] ** 2))
#             tot_lnlike += like
#     return tot_lnlike
...
pickle.dump(flat_samples, open(f'MCMC_results_{datetime.date.today()}_no_hera_mini_high_prior.pk', 'wb'))
