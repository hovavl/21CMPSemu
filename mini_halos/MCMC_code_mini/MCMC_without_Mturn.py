...
def lnprior(theta):
    ...
    if (-3.0 <= F_STAR10 <= -0.5 and -3.0 <= F_ESC10 <= 0.0 and 38 <= L_X <= 42
            ...
            and 0.1 <= NU_X_THRESH <= 1.5 and -0.5 <= ALPHA_STAR_MINI <= 0.5 and -4 <= F_ESC7_MINI <= 0):
        return 0.0
...
nwalkers = 24
niter = 10000
initial = np.array([-1.24,-2.0, 0.5, 0, -1.35, -2.5, 0.3, 40.0, 0.72])  # best guesses
ndim = len(initial)
...
pickle.dump(flat_samples, open(f'/gpfs0/elyk/users/hovavl/jobs/21cm_mcmc_job/MCMC_results_mini/low_prior_Fesc_MCMC_results_{datetime.date.today()}_with_hera_updated_mini_no_Mturn.pk', 'wb'))
...
plt.savefig(f'/gpfs0/elyk/users/hovavl/jobs/21cm_mcmc_job/MCMC_results_mini/low_prior_Fesc_with_hera_mini_{datetime.date.today()}_no_Mturn.png')
