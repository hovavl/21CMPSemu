...
def lnprior(theta):
    ...
    if (-3.0 <= F_STAR10 <= -0.0 and -3.0 <= F_ESC10 <= 0.0 and 38 <= L_X <= 42
            and -0.2 <= ALPHA_STAR <= 1 and -1 <= ALPHA_ESC <= 1 and -4.5 <= F_STAR7_MINI <= -1.0
...
pickle.dump(flat_samples, open(f'/gpfs0/elyk/users/hovavl/jobs/21cm_mcmc_job/MCMC_results_mini/MCMC_{datetime.date.today()}_small_f*7_all_fields.pk', 'wb'))
...
plt.savefig(f'/gpfs0/elyk/users/hovavl/jobs/21cm_mcmc_job/MCMC_results_mini/MCMC_{datetime.date.today()}_small_f*7_all_fields.png')
